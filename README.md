# Hi, I'm Sanjit Bhagat 👋

## 🚀 About Me

 DevOps Engineer passionate about automation, cloud technologies, CI/CD pipelines, and infrastructure management. Currently learning and building hands-on projects in Linux, Docker, Kubernetes, AWS, Terraform, and DevOps tools.

---

## 🛠️ Skills & Tools

### Operating System

* Linux (Ubuntu)
* Windows

### DevOps Tools

* Docker
* Kubernetes
* Jenkins
* Git & GitHub
* Terraform
* Ansible

### Cloud & Infrastructure

* AWS
* Networking Basics
* CI/CD Pipelines

### Programming & Scripting

* Python
* Bash Scripting

---

## 📌 Projects

### 🌱 Pollution Control by Identifying Potential Land for Afforestation

* Developed a data-driven solution for identifying suitable land areas for afforestation.
* Used Python for data analysis and visualization.
* Focused on environmental sustainability and pollution reduction.

---

## 📚 Currently Learning

* Linux Administration
* Docker & Kubernetes
* CI/CD Pipelines
* AWS Cloud
* Terraform & Infrastructure as Code
* Monitoring & Logging Tools

---

## 🎯 Goals

* Become a skilled DevOps Engineer
* Build scalable cloud infrastructure
* Automate deployment workflows
* Contribute to open-source projects

---

## 📫 Connect With Me

* GitHub: [https://github.com/](https://github.com/sanjit-bhagat)
* LinkedIn: [https://linkedin.com/](https://www.linkedin.com/feed/)

---

⭐ Always learning, building, and improving in the DevOps journey.
